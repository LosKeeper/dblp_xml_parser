...
void existence_auteur(int numero_auteur_un, int numero_auteur_deux) {
    if (numero_auteur_un == -1) {
        printf("L'auteur source (auteur un) n'existe pas dans cette base de "
               "donnée.\n");
    } else if (numero_auteur_deux == -1) {
        printf("L'auteur puit (auteur deux) n'existe pas dans cette base de "
               "donnée.\n");
    }
}
    ...
    while (graphe->liste_nb_liens[numero_auteur_un] != somme) {
        ...
        while ((int)graphe->liste_nb_liens[numero_auteur_buffer] !=
            ...
            if ((int)graphe->liste_sucesseurs[numero_auteur_buffer][k] ==
                ...
                chemin(tableau_dijkstra, numero_auteur_deux,
